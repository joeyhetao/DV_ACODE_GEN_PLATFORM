...
`docker-compose.yml` 的 `embedding_service` 已配 `env_file: .env`，会**自动**把 `.env` 全部变量注入容器，HuggingFace 库（FlagEmbedding 内部依赖的 `huggingface_hub`）启动时读到 `HF_ENDPOINT` 即走内网 mirror。**不需要改 compose 文件。**

> 验证：启动后 `docker compose logs embedding_service | grep -i "downloading\|hf-mirror"` 应看到从内网 mirror 拉取模型的日志。
...
# 此变量是 list[str] 类型，pydantic-settings 在 .env 里以 JSON 数组格式解析；
# 多个域名用 JSON 写法：ALLOWED_ORIGINS=["https://a.com","https://b.com"]
...
| `password authentication failed for user "dvuser"` | PG 凭证错误（`.env` 中 `DATABASE_URL` 密码与 PG 容器密码不一致） | 核对 `.env` 与 `docker-compose.yml` 中 PG 服务的 `POSTGRES_PASSWORD` |
| `dependency failed to start: container ... is unhealthy` | 上游容器（postgres / redis / qdrant / embedding_service）healthcheck 失败 | `docker compose logs <service>` 看具体哪个服务挂了；常见原因：磁盘满、模型下载失败、GPU 不可用 |
| `relation "templates" does not exist` | 表未创建（极少见，Base.metadata.create_all 失败） | `docker compose exec backend alembic upgrade head` 显式建表 |
