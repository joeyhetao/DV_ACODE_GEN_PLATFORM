...
**Step A · 预览（preview）**：系统返回 RAG Top-3 候选模板 + 每参数的预填值与来源标识（LLM / 正则 / 信号表 / 默认 / 占位）。

**Step B · 确认面板**：
- 默认套用置信度最高的候选；用户可一键切换其他候选，参数随之换为该候选的预填值
- 参数表单按 5 类源用不同颜色徽标提示来源；后端对 SV 标识符做了规范化清洗的字段显示"已规范化"角标，鼠标悬停可查看原始值；布尔/bins 表达式校验失败时实时显示红色错误提示
- 用户编辑参数 → 点击「确认生成」
...
**无匹配辅助（置信度 < 50%）**：
- 确认面板顶部展示提示条与「贡献新模板」入口，引导用户将该验证场景提交到模板库（见 §3.7）；用户仍可继续走 Step B + C 强行渲染最接近的候选作为草稿

**错误模式（用户可见）**：
- **HTTP 422 · off-topic**：dense 余弦闸判定非 IC 验证意图（详见 §3.8 / ARCHITECTURE §1.1）。前端弹"检测到非验证请求"专属 Modal，文案引导用户重新组织提问。
- **HTTP 503 · empty retrieval**：dense 闸通过但 RAG 三阶段 + 关键词补充召回后仍无候选——属于基础设施异常（Qdrant 不可达 / 模板库空 / embedding service 故障），不是用户问题。前端弹"暂时无法生成，请稍后重试或联系管理员"提示，用户无需自助处理，直接引导反馈给 SRE。两个错误模式在前端文案、错误码、可处置主体上严格分流。
